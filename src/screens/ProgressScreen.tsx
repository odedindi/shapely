    ...
    <div className="min-h-screen flex flex-col bg-[var(--color-surface)]">
      ...
      <div className="flex p-4 gap-4 bg-[var(--color-surface-alt)] border-b border-[var(--color-border)]">
        <div className="flex-1 flex flex-col items-center p-3 bg-[var(--color-surface)] rounded-xl border border-[var(--color-border)]">
          <span className="text-[10px] uppercase font-bold text-[var(--color-content-muted)] tracking-wider">Combos Seen</span>
          <span className="text-2xl font-black text-[var(--color-primary)]">{stats.combosSeen}</span>
        </div>
        <div className="flex-1 flex flex-col items-center p-3 bg-[var(--color-surface)] rounded-xl border border-[var(--color-border)]">
          <span className="text-[10px] uppercase font-bold text-[var(--color-content-muted)] tracking-wider">Avg Time</span>
          <span className="text-2xl font-black text-[var(--color-primary)]">{(stats.avgResponseTime / 1000).toFixed(1)}s</span>
        </div>
        <div className="flex-1 flex flex-col items-center p-3 bg-[var(--color-surface)] rounded-xl border border-[var(--color-border)]">
          <span className="text-[10px] uppercase font-bold text-[var(--color-content-muted)] tracking-wider">Accuracy</span>
      ...
      <main className="flex-1 overflow-auto p-4 relative">
        {loading ? (
          <div className="h-full flex items-center justify-center text-[var(--color-content-muted)]">Loading...</div>
        ) : activeShapeIds.length === 0 ? (
          <div className="h-full flex flex-col items-center justify-center text-[var(--color-content-muted)] gap-4">
            ...
            <p>Play some games to see your mastery heatmap!</p>
          </div>
        ) : (
      ...
      <AnimatePresence>
        {selectedCombo && (
          <m.div
            ...
            exit={{ opacity: 0, y: 50 }}
            className="fixed bottom-6 left-4 right-4 bg-[var(--color-surface-raised)] border border-[var(--color-border)] shadow-2xl p-4 rounded-2xl z-50 flex flex-col gap-3"
          >
            ...
            <div className="grid grid-cols-2 gap-4 mt-2">
              <div className="flex flex-col">
                <span className="text-[10px] text-[var(--color-content-muted)] uppercase tracking-wider font-bold">Accuracy</span>
              ...
              <div className="flex flex-col">
                <span className="text-[10px] text-[var(--color-content-muted)] uppercase tracking-wider font-bold">Avg Time</span>
                <span className="text-xl font-bold">
              ...
              <div className="flex flex-col">
                <span className="text-[10px] text-[var(--color-content-muted)] uppercase tracking-wider font-bold">Best Time</span>
                <span className="text-xl font-bold">
              ...
              <div className="flex flex-col">
                <span className="text-[10px] text-[var(--color-content-muted)] uppercase tracking-wider font-bold">Current Streak</span>
                <span className="text-xl font-bold">
